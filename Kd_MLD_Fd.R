...
# Merge SURFACE_PAR into NAG and filter profiles where it's within ±50% of modeled clear-sky PAR
NAG <- left_join(NAG, surface_PAR_df, by = "profile_number") %>%
  filter(!is.na(SURFACE_PAR)) %>%
...
# Final Kd and summary
profile_summary <- NAG %>%
  group_by(profile_number, Region, Season) %>%
  summarise(
    surface_PAR = mean(DOWNWELLING_PAR[PRES < 2], na.rm = TRUE),
    ...
    avg_transm_PAR = mean(transm_par, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  filter(!is.na(Kd_profile), Kd_profile >= 0,
         avg_CHLA >= 0, avg_Backscatter > 0)
