...
library(nortest)
ad_test <- ad.test(NAG_analysis_avg_depth_type$`median_bbp_chla`)
ad_test

# Perform Wilcoxon test on median_bbp_chla for difference between depth_type
wilcox_depth_type <- wilcox.test(median_bbp_chla ~ depth_type, data = NAG_analysis_avg_depth_type)

# Display the result of the Wilcoxon test
wilcox_depth_type

#Boxplot - Median bbp/chla in dcm vs in mixed layer

ggplot(NAG_analysis_avg_depth_type, aes(x = depth_type, y = median_bbp_chla, fill = depth_type)) +
  geom_boxplot() +
...
NAG_analysis_avg_depth_type_summary <- NAG_analysis_avg_depth_type %>%
  group_by(depth_type) %>%
  summarise(
...
# View summary table
NAG_analysis_avg_depth_type_summary
...
# Plot BBP/Chl-a at depth of MAXIMUM chl-a
# Boxplot of BBP/Chl-a at max [Chl-a] depth by season
# Subset data for NASTG region and remove outliers (bbp_chla_max_chla > 0.004)
NASTG_bbp_chla_dcm <- NAG_analysis_avg_dcm %>%
  filter(region == "NASTG", bbp_chla_max_chla <= 0.004)

# Create boxplot
ggplot(NASTG_bbp_chla_dcm, 
       aes(x = factor(season, levels = c("SPRING", "SUMMER", "AUTUMN", "WINTER")), 
           y = bbp_chla_max_chla, fill = season)) +
  geom_boxplot() +
  labs(
    x = "Season", 
    y = "BBP/Chl-a at max [Chl-a] depth"
  ) +
  scale_fill_manual(values = season_mapping) +
  theme_minimal() +
  theme(
...
# Kruskal-Wallis test for BBP/Chl-a at max Chl-a depth between seasons
kruskal_NASTG_bbp_chla_dcm <- kruskal.test(bbp_chla_max_chla ~ season, data = NASTG_bbp_chla_dcm)
kruskal_NASTG_bbp_chla_dcm

### non signifcant p=0.11

# Subset for NASTG region and filter out Qs ≤ 6 (outliers)
NASTG_bbp_chla_dcm <- NAG_analysis_avg_dcm %>%
  filter(region == "NASTG", Qs_max_chla < 6)

# Boxplot of Qs at max Chl-a depth by season
ggplot(NASTG_bbp_chla_dcm, 
       aes(x = factor(season, levels = c("SPRING", "SUMMER", "AUTUMN", "WINTER")), 
           y = Qs_max_chla, fill = season)) +
  geom_boxplot() +
  labs(
    x = "Season", 
    y = "Qs (mol Quanta m⁻² day⁻¹)"
  ) +
  scale_fill_manual(values = season_mapping) +
  theme_minimal() +
  theme(
...
# Spearman correlation between Qs at max Chl-a depth and BBP/Chl-a at max Chl-a depth
spearman_NASTG_bbp_chl_qs_dcm <- cor.test(
  NASTG_bbp_chla_dcm$Qs_max_chla,
  NASTG_bbp_chla_dcm$bbp_chla_max_chla,
  method = "spearman"
)
spearman_NASTG_bbp_chl_qs_dcm
rho_NASTG_bbp_chl_qs_dcm <- round(spearman_NASTG_bbp_chl_qs_dcm$estimate, 2)

# Scatter plot of BBP/Chl-a vs Qs at max Chl-a depth
ggplot(NASTG_bbp_chla_dcm, aes(x = Qs_max_chla, y = bbp_chla_max_chla, color = season)) +
  geom_point(alpha = 0.6, size = 3) +
  labs(
    x = "Qs (mol Quanta m⁻² day⁻¹)",
    y = "BBP/Chl-a "
  ...
  annotate("text",
           x = min(NASTG_bbp_chla_dcm$Qs_max_chla, na.rm = TRUE),
           y = max(NASTG_bbp_chla_dcm$bbp_chla_max_chla, na.rm = TRUE),
...
### SASTG ###
# Subset data for SASTG region and remove outliers (bbp_chla_max_chla > 0.010)
SASTG_bbp_chla_dcm <- NAG_analysis_avg_dcm %>%
  filter(region == "SASTG", bbp_chla_max_chla <= 0.010)
...
# Kruskal-Wallis test for BBP/Chl-a at max Chl-a depth between seasons
kruskal_SASTG_bbp_chla_dcm <- kruskal.test(bbp_chla_max_chla ~ season, data = SASTG_bbp_chla_dcm)
kruskal_SASTG_bbp_chla_dcm

# Also non significant p = 0.074
...
# Spearman correlation between Qs at max Chl-a depth and BBP/Chl-a at max Chl-a depth
spearman_SASTG_bbp_chl_qs_dcm <- cor.test(
  SASTG_bbp_chla_dcm$Qs_max_chla,
  SASTG_bbp_chla_dcm$bbp_chla_max_chla,
  method = "spearman"
)
spearman_SASTG_bbp_chl_qs_dcm
rho_SASTG_bbp_chl_qs_dcm <- round(spearman_SASTG_bbp_chl_qs_dcm$estimate, 2)

# Scatter plot of BBP/Chl-a vs Qs at max Chl-a depth
ggplot(SASTG_bbp_chla_dcm, aes(x = Qs_max_chla, y = bbp_chla_max_chla, color = season)) +
  geom_point(alpha = 0.6, size = 3) +
  labs(
    x = "Qs (mol Quanta m⁻² day⁻¹)",
    y = "BBP/Chl-a "
  ...
  annotate("text",
           x = min(SASTG_bbp_chla_dcm$Qs_max_chla, na.rm = TRUE),
           y = max(SASTG_bbp_chla_dcm$bbp_chla_max_chla, na.rm = TRUE),
           label = paste("ρ =", rho_SASTG_bbp_chl_qs_dcm),
           hjust = 0,
           vjust = 1,
           size = 5,
           color = "black") +
